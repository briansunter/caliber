...
function randomItem<T>(arr: T[]): T {
  ...
  if (item === undefined) throw new Error("randomItem: unexpected undefined");
  return item;
}
...
async function createTestLibrary(bookCount: number = 1_000_000) {
  ...
  for (const tag of TAGS.slice(0, 100)) {
    insertTag.run(tag);
    const row = db.query("SELECT id FROM tags WHERE name = ?").get(tag) as
      | { id: number }
      | undefined;
  ...
  for (let batch = 0; batch < totalBatches; batch++) {
    ...
    db.transaction(() => {
      for (let i = batchStart; i <= batchEnd; i++) {
        ...
        if (!authorIds.has(author.name)) {
          ...
          const row = db.query("SELECT id FROM authors WHERE name = ?").get(author.name) as
            | { id: number }
            | undefined;
        ...
        if (Math.random() > 0.5) {
          ...
          if (!seriesIds.has(seriesName)) {
            ...
            const row = db.query("SELECT id FROM series WHERE name = ?").get(seriesName) as
              | { id: number }
              | undefined;
        ...
        if (Math.random() > 0.3) {
          ...
          if (!publisherIds.has(publisherName)) {
            ...
            const row = db.query("SELECT id FROM publishers WHERE name = ?").get(publisherName) as
              | { id: number }
              | undefined;
        ...
        for (const tagName of bookTags) {
          ...
          if (!tagId) {
            ...
            const row = db.query("SELECT id FROM tags WHERE name = ?").get(tagName) as
              | { id: number }
              | undefined;
            if (row) {
              tagId = row.id;
    ...
    process.stdout.write(
      `\r📚 Batch ${batch + 1}/${totalBatches}: ${batchEnd.toLocaleString()} books | ${rate.toLocaleString()} books/sec | ETA: ${remaining}s`,
    );
  }
